...
const CEC_ARTICLE_LINK = (document.body && document.body.dataset.newsArticleLink) || './news-article.html?slug=';
...
const galleryItems = [
  ...
  { image: '../images/RUSIZI1.jpg', name: 'Rusizi Port - Photo 1', category: 'Marine & Civil Works' },
  { image: '../images/RUSIZI2.png', name: 'Rusizi Port - Photo 2', category: 'Marine & Civil Works' },
  { image: '../images/RUSIZI3.jpg', name: 'Rusizi Port - Photo 3', category: 'Marine & Civil Works' },
  { image: '../images/RUSIZI4.png', name: 'Rusizi Port - Photo 4', category: 'Marine & Civil Works' },
  { image: '../images/RUSIZI6.jpg', name: 'Rusizi Port - Photo 5', category: 'Marine & Civil Works' },
  { image: '../images/RUSIZI7.jpg', name: 'Rusizi Port - Photo 6', category: 'Marine & Civil Works' },
  { image: '../images/RUSIZI8.jpg', name: 'Rusizi Port - Photo 7', category: 'Marine & Civil Works' },
  { image: '../images/RUSIZI9.jpg', name: 'Rusizi Port - Photo 8', category: 'Marine & Civil Works' },
...
function renderNewsPage() {
  ...
  const render = () => {
    ...
    results.innerHTML = matches.map(article => `<article class="news-card"><a class="news-card-image" href="${CEC_ARTICLE_LINK}${encodeURIComponent(article.slug)}"><img src="${escapeHtml(resolveCecImage(article.image))}" alt="${escapeHtml(article.imageAlt)}" loading="lazy"></a><div class="news-card-body"><div class="news-card-meta"><span class="news-card-category">${escapeHtml(article.categoryLabel)}</span>${article.date ? `<time class="news-card-date"><i class="bi bi-calendar3"></i>${escapeHtml(article.date)}</time>` : ''}</div><h2>${escapeHtml(article.title)}</h2><p>${escapeHtml(article.excerpt)}</p><a class="news-card-link" href="${CEC_ARTICLE_LINK}${encodeURIComponent(article.slug)}">Read More →</a></div></article>`).join('');
...
function openLightbox(index) {
  ...
  image.src = resolveCecImage(item.image); image.alt = item.name; lightbox.querySelector('strong').textContent = item.name; lightbox.querySelector('figcaption span').textContent = item.category;
...
function renderArticlePage() {
  ...
  const image = document.querySelector('#article-image'); image.src = resolveCecImage(article.image); image.alt = article.imageAlt;
